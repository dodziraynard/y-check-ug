...
import { get_shs_schools,delete_shs_school} from '../../actions/SchoolActions';
...
const SHSTableList = () => {
  ...
  // GET THE ALL  SCHOOLS
  const shs_school_delete = useSelector(state => state.shs_school_delete);
  const { delete_school } = shs_school_delete;
      ...
      <div className='patient-table basic_table'>
        <TableContainer component={Paper} style={{ boxShadow: '0px 4px 6px rgba(0, 0, 0, 0.1)' }}>
          ...
          <Table sx={{ minWidth: 650 }} aria-label="simple table" style={{ marginLeft: '1rem' }}>
            <TableHead>
              <TableRow>
                <TableCell>Senior High School Name</TableCell>
                <TableCell>Action</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {shs_schools.map((school) => (
