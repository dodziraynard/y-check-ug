...
function StudyPhaseSummaryFlag({ study_phase }) {
    ...
    const { trigger: getFlags, data: responseData, error, isLoading } = useAxios({ mainUrl: `${BASE_API_URI}/${pid}/${study_phase ?? adolescent?.study_phase}/summary-flags/` });
    ...
    useEffect(() => {
        if (adolescent) {
            getFlags(`${BASE_API_URI}/${pid}/${study_phase ?? adolescent?.study_phase}/summary-flags/`)
        }
    }, [adolescent])
    ...
    const handleOverrideFormSubmit = (event) => {
        ...
        const response = postFlagColorOverride(
            `${BASE_API_URI}/${pid}/${study_phase ?? adolescent?.study_phase}/summary-flags/`,
            body)
        if (response) {
            getFlags(`${BASE_API_URI}/${pid}/${study_phase ?? adolescent?.study_phase}/summary-flags/`)
        ...
        <Fragment>
            ...
            <div className="patients-widget">
                <section className="page-summary">
                    <div style={{ position: "sticky", top: "4em", background: "white", zIndex: "3" }}>
...
export default StudyPhaseSummaryFlag;
