...
def compute_grip_test(adolescent: Adolescent,
                      study_phase: str,
                      for_right_arm=False) -> int:
    ...
    if not (group and adolescent and adolescent.gender):
        return 0
    grip_test_result = group.get_group_value(adolescent, study_phase)
    ...
    if recommended_value and recommended_value[
            for_right_arm] > grip_test_result:
        return -1
    return 1


def compute_bmi_sd_function(adolescent: Adolescent,
                            study_phase: StudyPhase) -> int:
    ages_in_months_array = [
        10 * 12 + 6, 11 * 12 + 6, 12 * 12 + 6, 13 * 12 + 6, 14 * 12 + 6,
        15 * 12 + 6, 16 * 12 + 6, 17 * 12 + 6, 18 * 12 + 6
    ]
    female_bmi_matrix = [
        #   -3     -2    1     2
        [12.5, 13.7, 19.4, 23.1],
    ...
    if height_question and weight_question:
        height_value = height_question.get_response(adolescent,
                                                    study_phase,
                                                    numeric=True)
        weight_value = weight_question.get_response(adolescent,
                                                    study_phase,
                                                    numeric=True)
    ...
    sds = female_bmi_matrix[
        index] if adolescent.gender == "female" else male_bmi_matrix[index]
    sd_index = [-3, -2, 1, 2, 3][bisect_left(sds, bmi)]
    interpretations = [
        "Thinness", "Thinness", "Normal", "Overweight", "Obesity", "Obesity"
    ]
    ...
    summary_flag = SummaryFlag.objects.filter(adolescent=adolescent,
...
def compute_anaemia_status(adolescent: Adolescent,
                           study_phase: StudyPhase) -> AnaemiaStatus:
    ...
    value = anaemia_question.get_response(adolescent,
                                          study_phase,
                                          numeric=True)  # in g/dL
    if not value:
        return AnaemiaStatus.NORMAL
...
def compute_time_difference(adolescent: Adolescent, question1: Question,
                            question2: Question,
                            study_phase: StudyPhase) -> int:
    value1 = question1.get_response(adolescent, study_phase,
                                    numeric=True)  # e.g., 20 for 8pm
    value2 = question2.get_response(adolescent, study_phase,
                                    numeric=True)  # e.g., 4 for 4am
...
def compute_vision_status(adolescent: Adolescent, question: Question,
                          study_phase: StudyPhase) -> str:
    value = question.get_response(adolescent, study_phase)
    if not value:
        return "Unknown"
    ...
    if (dist_numerator == 6 and dist_denominator in [3, 3.8, 4.8, 6, 7.5, 9.5]
        ) or (dist_denominator == 6
              and dist_numerator in [3, 3.8, 4.8, 6, 7.5, 9.5]):
        return "Normal"
    elif (dist_numerator in [12, 15, 18]) or (dist_denominator in [12, 15, 18
                                                                   ]):
        return "Mild"
    elif (dist_numerator in [19, 24, 30, 38, 48, 60]) or (dist_denominator in [
            19, 24, 30, 38, 48, 60
    ]):
        return "Moderate"
    elif (dist_numerator in [60, 120, 240, 600] and dist_denominator == 6) or (
            dist_denominator in [60, 120, 240, 600] and dist_numerator
            == 6) or (dist_numerator == 3 and dist_denominator == 60) or (
                dist_denominator == 3 and dist_numerator == 60):
        return "Severe"
    elif (dist_numerator == 3
          and dist_denominator == 60) or (dist_numerator == 3
                                          and dist_denominator == "*"):
        return "Blindness"
    else:
        return "Unknown"
