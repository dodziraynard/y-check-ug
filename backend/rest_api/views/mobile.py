...
from dashboard.models import (Adolescent, CheckupLocation, AdolescentResponse,
                              Option, AdolescentActivityTime)

from rest_api.serializers import (CheckupLocationSerializer,
                                  AdolescentSerializer, QuestionSerializer,
                                  SectionSerializer, ResponseSerialiser,
                                  AdolescentResponseSerialiser,
                                  MobileConfigSerializer, RegisterSerializer,
                                  UserSerializer)
...
class GetCheckupLocation(generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        response_data = {
            "checkup_locations": self.serializer_class(locations,
                                                       many=True).data,
        }
...
class MobileAdolescentsAPI(generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if query:
            adolescents = adolescents.filter(
                Q(pid__icontains=query) | Q(surname__icontains=query)
                | Q(other_names__icontains=query))
        adolescents = apply_filters(adolescents, filters)

        response_data = {
            "adolescents":
            self.serializer_class(adolescents,
                                  context={
                                      "request": request
                                  },
                                  many=True).data,
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not pid:
            ...
            response_data = {"error_message": f"Invalid PID '{pid}'"}
            return Response(response_data)
        ...
        try:
            ...
            logger.error("Error occured while adding/updating adolescent: %s",
                         str(e))
            ...
            response_data = {
                "adolescent":
                AdolescentSerializer(adolescent, context={
                    "request": request
                }).data,
                "error_message":
                "",
                "message":
                "Adolescent created successfully.",
            }
        return Response(response_data)
...
class GetNextAvailableQuestions(generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Exclude sections not availble for this phase.
        ...
        if query:
            target_questions = target_questions.filter(
                Q(text__icontains=query) | Q(question_id__icontains=query))
        ...
        for index, question in enumerate(target_questions):
            ...
            if question.has_previous_question_requirement(
                    current_question) and index > len(invalid_questions_ids):
            ...
            if not question.are_previous_response_conditions_met(
                ...
                if index > len(invalid_questions_ids) and action in [
                        "next", "next_unanswered"
                ]:
                    last_invalid_question_number = question.number
        ...
        questions = target_questions.exclude(
            Q(id__in=invalid_questions_ids)
            | Q(number__gte=last_invalid_question_number))
        new_section = None
        ...
        response_data = {
            ...
            QuestionSerializer(questions,
                               ...
                               many=True).data if questions else None,
            "new_section":
            ...
            AdolescentResponseSerialiser(
                responses, context={
                    "request": request
                }, many=True).data if responses else None,
        }
        return Response(response_data)
...
class PostMutipleResponses(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        response_data = {
            ...
            last_answered_question.id if last_answered_question else "-1",
        }
        return Response(response_data)
...
class RecordAdolescentActivityTime(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not (activity_tag and timestamp):
            return Response(
                {"error_message": "activity_tag and timestamp are required."})

        activity_record = AdolescentActivityTime.objects.filter(
            adolescent=adolescent, activity_tag=activity_tag).first()
        if not activity_record:
            AdolescentActivityTime.objects.create(adolescent=adolescent,
                                                  timestamp=timestamp,
        ...
        response_data = {"message": "Time recorded"}
        return Response(response_data)
